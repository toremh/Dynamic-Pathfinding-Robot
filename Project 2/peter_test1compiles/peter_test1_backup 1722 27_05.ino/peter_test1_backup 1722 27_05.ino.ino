...
//#include <cmath>    //needed to be able to use atan2 and M_Pi
...
void TimerHandler1() {
  ...
  if (timerCounter == TIMER_ITERATION_COUNT) {
    ...
    */
    ...
    if (distPhotoLL <= 500) {distPhotoLL = 9998; }
    else if (distPhotoLL >= 2000) {distPhotoLL = 9999;}
    ...
    if (distPhotoL <= 100) {distPhotoL = 9998; }
    else if (distPhotoL >= 700) {distPhotoL = 9999;}
    ...
    if (distPhotoR <= 100) {distPhotoR = 9998; }
    else if (distPhotoR >= 700) {distPhotoR = 9999;}
    ...
    if (distPhotoRR <= 500) {distPhotoRR = 9998; }
    else if (distPhotoRR >= 2000) {distPhotoRR = 9999;}
...
//getting unstuck when between three things or been staring at the wall a long time. Not sure if i should make the wall thing separate.
//issue, I can call get unstuck but it won't stay called, will just move until no longer satisfying the condition of looking at 3 things.
actions getUnstuck(int distUltra, int distIRFL, int distIRFR, int distIRR, int distIRL, int angGyro) {
...
actions avoid(int distUltra, int distIRFL, int distIRFR, int distIRR, int distIRL) {
  ...
  if (objectsOnThreeSides()) {
    ...
    unstuckCounter = 0;
    return getUnstuck(distUltra, distIRFL, distIRFR, distIRR, distIRL, angGyro);
  }
    ...
    if (distIRL - distIRR >= DEADZONE) {
      ...
      Serial1.print("in a tight space");
      position = TIGHTSPACE;
    }
...
int fireDirection(int distPhotoLL, int distPhotoL, int distPhotoR, int distPhotoRR, int pt_space) {
  ...
  //finding the degrees needed:
  //float avg_ang_far = atan2(distPhotoLL - distPhotoRR, pt_space) * 180 / M_PI;
  float avg_pos_close = (-0.5 * normL + 0.5 * normR) * pt_space;
  //float avg_ang_close = atan2(-0.5 * normL + 0.5 * normR, pt_space) * 180 / M_PI;
  ...
  // }
  //alternative making use of the angle close variables made
  //if (distPhotoL < 300 || distPhotoR < 300) {
  //int direction = (int)(avg_pos_close);
  //return direction;
  //}


  int direction = 1;  //is direction a range of ints up to 360? Or do we want to do directions within say 5 degree blocks.
  return direction;
...
actions firePosSimple(int distPhotoLL, int distPhotoL, int distPhotoR, int distPhotoRR) {
  ...
  if (distPhotoLL != 9999 && distPhotoRR != 9999 && distPhotoLL != 9998 && distPhotoRR != 9998) {
    if (distPhotoLL - distPhotoRR >= 150) {
      //turn right
      Serial1.println("Turn right");
      turningSpeed = 20;
      return CW;
    } else if (distPhotoRR - distPhotoLL >= 150) {
      //turn Left
...
void control() {
  ...
  if (photoCheckL == 1 and photoCheckR == 1) {
    Serial1.print("enters extinguish ");
    motors(extinguish());
    ...
    //seek(fireDirection);
    Serial1.print(", control loop calls seek ");
    motors(firePosSimple(distPhotoLL, distPhotoL, distPhotoR, distPhotoRR));
    //motor(seek)());
  }
}
